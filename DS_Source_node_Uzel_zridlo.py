# Vykresli 3 fazove portrety typu Uzel-Zridlo
...
plt.title('Fázový portrét lineárního systému: Uzel-zřídlo pro 2 ruzna vlastni $\lambda >0$ ', fontsize=12)
...
plt.xlabel('$x$')
plt.ylabel('$y$')
plt.title(r'Fázový portrét (Uzel-zřídlo) generovaný čistě maticově $J \cdot \vec{x}$ pro $\lambda = 3$')
plt.grid(True, linestyle=':', alpha=0.5)
...
plt.xlabel('Stavová proměnná $x$')
plt.ylabel('Stavová proměnná $y$')
plt.title('Fázový portrét lineárního systému: Uzel-zřídlo pro $\lambda = 3$ ', fontsize=12)
# plt.legend(loc='upper right')
plt.grid(True, linestyle=':', alpha=0.5)
